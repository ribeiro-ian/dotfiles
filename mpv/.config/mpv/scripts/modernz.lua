...
mp.register_event("file-loaded", function()
    ...
    check_path_url()
    if user_optsmatickeyframemode then
        user_opts.seekbarkeyframes = mp.get_property_number("duration", 0) > user_optsmatickeyframelimit
    end
...
-- ───── ADDED FOR COLORSCHEME PLUGIN INTEGRATION ─────

-- Script message for colorscheme reloading
mp.register_script_message("modernz-reload", function()
    opt.read_options(user_opts, "modernz")
    local colorscheme_path = mp.command_native({"expand-path", "~~/script-opts/modernz-colorscheme.conf"})
    local f = io.open(colorscheme_path, "r")
    if f then
        f:close()
        opt.read_options(user_opts, "modernz-colorscheme")
    end
    set_osc_styles()
    set_icon_theme()
    if state.osc_visible then
    ...
    msg.info("ModernZ reloaded")
end)

-- Load auto-generated colorscheme if exists
local colorscheme_path = mp.command_native({"expand-path", "~~/script-opts/modernz-colorscheme.conf"})
local f = io.open(colorscheme_path, "r")
if f then
    f:close()
    opt.read_options(user_opts, "modernz-colorscheme")
    set_osc_styles()  -- reaplica os estilos com as novas cores
    msg.info("Loaded colorscheme override")
end